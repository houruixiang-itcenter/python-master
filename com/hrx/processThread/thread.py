...
t = threading.Thread(target=loop, name="TestThread")
t.start()
t.join()
print("ending")
...
# todo 多进程由于会同时操作同一块数据 所以必然会有线程安全出现
def change(n):
    global cur
    cur = cur + n
    cur = cur - n
...
t1 = threading.Thread(target=run_loop, args=(2,))
t2 = threading.Thread(target=run_loop, args=(10,))
t1.start()
t2.start()
t1.join()
t2.join()
...
# 初始值 cur = 0
#
# t1: x1 = cur + 2  # x1 = 0 + 2 = 2
#
# t2: x2 = cur + 10  # x2 = 0 + 10 = 10
# t2: cur = x2      # cur = 10
#
# t1: cur = x1      # cur = 2
# t1: x1 = cur - 2  # x1 = 2 - 2 = 0
# t1: cur = x1      # cur = 0
#
# t2: x2 = cur - 10  # x2 = 0 - 10 = -10
# t2: cur = x2   # cur = -10
...
#
# 两个线程同时一存一取，就可能导致余额不对，你肯定不希望你的银行存款莫名其妙地变成了负数，所以，我们必须确保一个线程在修改balance的时候，
# 别的线程一定不能改。
#
