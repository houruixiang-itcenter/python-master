...
from com.hrx.ood.Test import Ood_test1, Ood_test, Chain, TT
...
class Ood_Height:
	...
	# c = Chain()
	# todo 其实上述就是把一些方法或者属性动态化了  所谓动态化 就是说属性可以自己定义  方法可以自己配置实现 仅此而已
	# todo 这种动态化 可以根据实际需求 进行配置
	# todo  比如动态生成 需求的url
	# h = Chain().users
	# h1 = t('michael')
	...
	# todo 这几个方法作用于可迭代对象  __setitem__设置元素  __delitem__删除元素 __getitem__获取元素
	# todo 只有在没有找到属性的情况下，才调用__getattr__，已有的属性，比如name，不会在__getattr__中查找。其实就是一个容错处理
	# todo __call__ 调用实例本身时候 类似  s = xxx() s()时候调用
	...
	tt = TT("xte")
	tt()  # 调用call
	# todo 其实上面是一个函数调用  但是 看起来好像一个函数调用啊   mmp完全模糊了函数和对象的边界
	# py 提供了 这种傻傻的分不清函数 还是实例的Api: 其实就是 类/实例 --- True  函数 --- False
	print("------------------------------------__call__-----------------------------------------------------")
	print(callable(TT("xte")))
	print(callable(Ood_Height()))
	print(callable(Ood_test1))
